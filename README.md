...
このポートフォリオサイト自身のリポジトリです。Next.js (App Router) と GitHub REST API で public リポジトリを自動取得し、各リポジトリの README から概要・使用技術・特徴を抽出してカード表示します。ISR により1時間ごとにバックグラウンドで再生成し、訪問者には常にキャッシュ済みの高速なページを配信します。README を更新するだけで掲載内容が自動反映される設計と、Claude Code による AI 駆動開発で構築した点が力点です。
